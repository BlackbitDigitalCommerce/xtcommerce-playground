...
class themes_history extends xt_backend_cls {
	...
	function _getParams() {
        ...
        if(!$this->url_data['edit_id'] && $this->url_data['new'] != true) {
            ...
            $js .= "Ext.Msg.show({
			   title:'" . TEXT_THEME_RESTORE . "',
			   msg: '" . TEXT_THEME_RESTORE_DIALOG . "',
			   buttons: Ext.Msg.YESNO,
			   animEl: 'elId',
				 fn: function(btn) {if (btn == 'yes') {addTab('row_actions.php?type=restore_theme&seckey=" . _SYSTEM_SECURITY_KEY . "&id='+edit_id+'&theme='+edit_theme,'".TEXT_THEME_RESTORE."');}},
            ...
            $js = "function runImport(edit_id,btn){
	  		var edit_id = edit_id;
	  		if (btn == 'yes') {
	  			addTab('row_actions.php?type=api_csv_export&seckey="._SYSTEM_SECURITY_KEY."&id='+edit_id,'".TEXT_THEME_RESTORE."');  
			}
