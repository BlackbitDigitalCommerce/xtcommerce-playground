...
checkHttps              => '. ($checkHttps ? 'true' : 'false');
    ...
    function _set($data, $set_type = 'edit')
    {
        ...
        if ($set_type != 'new') {
            ...
            if ($data['shop_id'] > 0) {
                ...
                if (/*multi_store_check_max_stores($rc_count)*/ true) {
                    ...
                    $db->Execute("CREATE TABLE " . TABLE_CONFIGURATION_MULTI . $objS->new_id . " LIKE " . TABLE_CONFIGURATION_MULTI . "1");
                    $db->Execute("INSERT INTO " . TABLE_CONFIGURATION_MULTI . $objS->new_id . " select * FROM " . TABLE_CONFIGURATION_MULTI . "1");

                    $this->copyPermissions(TABLE_CONTENT_PERMISSION, $objS->new_id);
                    $this->copyPermissions(TABLE_PRODUCTS_PERMISSION, $objS->new_id);
                    $this->copyPermissions(TABLE_CATEGORIES_PERMISSION, $objS->new_id);
                    $this->copyTableValuesByShop(TABLE_CONFIGURATION_PAYMENT, $objS->new_id);
                    if (StoreIdExists(TABLE_PAYMENT_DESCRIPTION, 'payment_description_store_id'))
                        $this->copyTableValuesByShop(TABLE_PAYMENT_DESCRIPTION, $shop_id, '', 'payment_description_store_id');
                    $this->copyTableValuesByShop(TABLE_PLUGIN_CONFIGURATION, $objS->new_id);
                    if (StoreIdExists(TABLE_PRODUCTS_DESCRIPTION, 'products_store_id'))
                        $this->copyTableValuesByShop(TABLE_PRODUCTS_DESCRIPTION, $shop_id, '', 'products_store_id');
                    if (StoreIdExists(TABLE_CONTENT_ELEMENTS, 'content_store_id'))
                        $this->copyTableValuesByShop(TABLE_CONTENT_ELEMENTS, $shop_id, '', 'content_store_id');
                    if (StoreIdExists(TABLE_MANUFACTURERS_DESCRIPTION, 'manufacturers_store_id'))
                        $this->copyTableValuesByShop(TABLE_MANUFACTURERS_DESCRIPTION, $shop_id, '', 'manufacturers_store_id');
                    if (StoreIdExists(TABLE_CATEGORIES_DESCRIPTION, 'categories_store_id'))
                        $this->copyTableValuesByShop(TABLE_CATEGORIES_DESCRIPTION, $shop_id, '', 'categories_store_id');
                    if (StoreIdExists(TABLE_PRODUCTS_TO_CATEGORIES, 'store_id'))
                        $this->copyTableValuesByShop(TABLE_PRODUCTS_TO_CATEGORIES, $shop_id, '', 'store_id');


                    if (StoreIdExists(TABLE_SEO_URL, 'store_id'))
                        $this->copySeoValuesByShop(TABLE_SEO_URL, $objS->new_id);
                    ...
                    $this->copyTableValuesByShop(TABLE_CONFIGURATION_LANG_MULTI, $objS->new_id, 1, 'store_id', '', 'language_content_id');
        ...
        if ($this->url_data['new'] == 'true' && $this->url_data['save'] == 'true') {
            $obj->message = __define('TEXT_STORE_CREATED');
        }
