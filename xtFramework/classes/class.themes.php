...
';
	...
	function _getParams() {
		...
		if($this->url_data['pg']=='overview' && !$this->url_data['edit_id'] && $this->url_data['new'] != true){
            ...
            $rowActions[] = array('iconCls' => 'generate_css', 'qtipIndex' => 'qtip1', 'tooltip' => TEXT_THEME_GENERATECSS);
            if ($this->url_data['edit_id'])
                $js = "var edit_id = ".$this->url_data['edit_id'].";";
            else
                $js = "var edit_id = record.id;";
            $js.= "Ext.Msg.confirm('".TEXT_THEMES."','".TEXT_THEME_GENERATECSS_DIALOG."',function(btn){doCssGeneration(edit_id,btn);})";
            $rowActionsFunctions['generate_css'] = $js;

            $rowActions[] = array('iconCls' => 'plus', 'qtipIndex' => 'qtip1', 'tooltip' => TEXT_THEME_CREATE_OVERRIDE);
            if ($this->url_data['edit_id'])
                $js = "var edit_id = ".$this->url_data['edit_id'].";";
            else
                $js = "var edit_id = record.id;";
            $js.= "Ext.Msg.confirm('".TEXT_THEMES."','".TEXT_THEME_CREATE_OVERRIDE_DIALOG."',function(btn){doCreateTemplateOverride(edit_id,btn);})";
            $rowActionsFunctions['plus'] = $js;

			$rowActions[] = array('iconCls' => 'copy', 'qtipIndex' => 'qtip1', 'tooltip' => TEXT_THEME_COPY);
			if ($this->url_data['edit_id'])
				$js = "var edit_id = ".$this->url_data['edit_id'].";";
			else
				$js = "var edit_id = record.id;";
			$js.= "Ext.Msg.confirm('".TEXT_THEMES."','".TEXT_THEME_COPY_DIALOG."',function(btn){doThemeCopy(edit_id,btn);})";
			$rowActionsFunctions['copy'] = $js;

            $rowActions[] = array('iconCls' => 'theme_history', 'qtipIndex' => 'qtip1', 'tooltip' => TEXT_THEME_HISTORY);
            if ($this->url_data['edit_id'])
            ...
            $js .= "addTab('adminHandler.php?load_section=themes_history&theme_dir='+edit_id,'".TEXT_THEME_HISTORY." ('+edit_id+')', 'theme_history'+edit_id)";
	...
	private function getLessConfig($theme,$return_tabs=false) {
		...
		if (is_array ( $xml )) {
            ...
            !isset($xml ['css_file'])) {
                die(TEXT_THEME_ERROR_NO_CONFIG_FILE);
            }
			...
			if ($xml ['less_support']=='true') {
				...
				die(TEXT_THEME_ERROR_NO_LESS_SUPPORT);
			}
		} else {
			die(TEXT_THEME_ERROR_NO_CONFIG_FILE);
		}
        ...
        if (is_file ( $file ))
        {
            ...
            if (self::_VALIDATE_XML)
            {
                ...
                if (is_file($xsd))
                {
                    ...
                    if($valid['valid'] != true)
                    {
                        $theme_data['validation']['messages'][] = ['message' => TEXT_THEME_XML_INVALID, 'type' =>'error'];
                    }
                }
                else {
                    $theme_data['validation']['messages'][] = ['message' => TEXT_THEME_WARNING_XSD_MISSING, 'type' =>'warning'];;
                }
            }
    ...
    public function createOverride($theme)
    {
        ...
        {
            $this->msg(TEXT_THEME_ERROR_OVERRIDE_FROM_OVERRIDE, 'error');
            die();
        }
        ...
        if($theme_data) // theme.xml exists
        {
            if($theme_data['less_support']
                ...
                )
                {
                    ...
                    {
                        $this->msg(TEXT_THEME_OVERRIDE_LESS_FILE_CREATED);
                    }
                    else
                    {
                        $full_success = false;
                        $this->msg(TEXT_THEME_ERROR_CREATE_FILE . ': ' . $less_main_file, 'warning');
                    }
                    ...
                    {
                        $this->msg(TEXT_THEME_OVERRIDE_VARIABLES_FILE_CREATED);
                    }
                    else
                    {
                        $full_success = false;
                        $this->msg(TEXT_THEME_ERROR_CREATE_FILE . ': ' . $less_variable_file, 'warning');
                    }
                    ...
                    if($full_success) $this->msg('<b style="font-size: 1.1em; color: #000">'.TEXT_THEME_REQUEST_CSS_GENERATION.'</b>', 'info');
                    ...
                    $this->msg(TEXT_THEME_ERROR_MISSING_CONFIG, 'warning');
                }
            }
            else {
                $this->msg(TEXT_THEME_WARNING_NO_LESS_SUPPORT, 'warning');
            }
        }
        else $this->msg(TEXT_THEME_WARNING_THEME_XML_NOT_FOUND, 'warning');
    }
        ...
        if($theme_data) // theme.xml exists
        {
            ...
            if($theme_data['less_support']
                ...
                if($theme_data['less_folder']
                    ...
                    echo(TEXT_THEME_ERROR_MISSING_CONFIG);
                }
            }
            else {
                echo(TEXT_THEME_WARNING_NO_LESS_SUPPORT);
            }
        }
        else $this->msg(TEXT_THEME_WARNING_THEME_XML_NOT_FOUND, 'warning');
    ...
    public function restore_theme($theme, $restore_file)
    {
        ...
        if (file_exists($less_variable_file) && file_exists($less_restore_file)) {
            ...
            $this->msg(TEXT_THEME_RESTORE_SUCCESS);

        } else {
            $this->msg(TEXT_THEME_RESTORE_ERROR, 'warning');
        }
    }
	...
	public function generateCss($theme)
    {
		...
		if(is_array($theme_data))
        {
                ...
                try
                {
                    ...
                    foreach ($theme_data['less_import_directories'] as $key => $val)
                    {
                        ...
                        if(false && !is_dir($dir))
                        {
                            ...
                            if(!is_dir($dir))
                            {
                                $this->msg(TEXT_THEME_WARNING_INVALID_DIRECTORY . '<br>' . $dir, 'warning');
                            }
                        }
                        $directories[$dir]='';
                    }
                    ...
                    $this->msg(TEXT_THEME_GENERATECSS_SUCCESS);
            ...
            $this->msg(TEXT_ERROR_THEME_DONT_EXIST.'<br>'.$theme, 'error');
		    die ();
        }
	}
