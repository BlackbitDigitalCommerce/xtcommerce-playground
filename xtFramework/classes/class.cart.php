		...
		if (isset($quantites) && is_array($quantites))
		{
			foreach ($quantites as $key => $q)
			{
				...
				if($content_product->is_product!=true)
				{
					...
					// meldung ERROR_STOCK_REDUCED_AMOUNT nur wenn > 0, ohne check kam zB '..wurde um 3 stück reduziert' und gleichzeitig '..wurde um 0 stück reduziert'
					if($new_stock>0) {
					    $info->_addInfo(sprintf(ERROR_STOCK_REDUCED_AMOUNT, $pname, $new_stock), 'info');
                    }
					continue;
				}
