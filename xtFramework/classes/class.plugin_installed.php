...
class plugin_installed extends plugin{
	...
	function _getParams() {
        ...
        // plugin changelog
        $rowActions[] = array('iconCls' => 'PLUGIN_CHANGELOG', 'qtipIndex' => 'qtip1', 'tooltip' => TEXT_PLUGIN_CHANGELOG);
	    ...
	    $rowActions[] = array('iconCls' => 'deletePlugin', 'qtipIndex' => 'qtip1', 'tooltip' => __define('TEXT_PLUGIN_DELETE'));
        ...
        $jsUpdate="var record_ids = edit_id;".
        		...
        		" });".
        		"}";
        $rowActions[] = array('iconCls' => 'update_translations', 'qtipIndex' => 'qtip1', 'tooltip' => __define('TEXT_UPDATE_TRANSLATIONS'));
	...
	function _setStatus($id, $status, $is_installer = false) {
        ...
        if($iskco)
        {
            if (!$is_pro_version)
            {
                ...
                {
                    throw new Exception(ERROR_PLUGIN_CANNOT_BE_CHANGED);
                }
                $status = 1;
            }
            else
            {
                ...
                {
                    throw new Exception(TEXT_ERROR_PLUGIN_ONLY_IN_FREE_VERSION);
                }
                $status = 0;
            }
        }
