...
class payment extends xt_backend_cls {
	...
	*/
	...
	function _getParams() {
		...
		define('TEXT_PAYMENT_NAME_STORE'.$store_handler->shop_id, __define('TEXT_PAYMENT_NAME'));
		...
		$js .= "addTab('adminHandler.php?load_section=payment_price&pg=overview&payment_id='+edit_id,'".TEXT_PAYMENT_PRICE." '+edit_name,'payment_price_'+edit_id)";
	...
	function setPaymentConfig($data) {
		...
		foreach ($stores as $sdata) {
			...
			while (!$record->EOF) {
				...
				$db->Execute(
					"UPDATE ".TABLE_CONFIGURATION_PAYMENT." SET config_value = CASE WHEN type='status' AND ?='' THEN 0 ELSE ? END   WHERE payment_id=? and shop_id=? and config_key=?",
					array($conf_value, $conf_value, $payment_id, $sdata['id'], $record->fields['config_key'])
				);
				$record->MoveNext();
			}
