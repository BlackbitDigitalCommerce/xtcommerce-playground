...
class customer extends check_fields{
	...
	function _registerCustomer($data, $register_type='both', $add_type = 'insert', $check_data=true, $login_customer=true){
		...
		if(is_array($data['cust_info'])){
            ...
            {
                $this->error = true;
                $info->_addInfo(constant('TEXT_NO_GUEST_ALLOWED_WITH_VAT'));
            }
	...
	function _checkCustomerData($data, $add_type = 'insert', $check_data=true){
		...
		if($check_data == true){
			...
			$this->_checkMatch($data['customers_email_address'], $data['customers_email_address_confirm'], ERROR_EMAIL_ADDRESS_NOT_MATCHING, true);
			...
			else
			{
			    ...
			    $reset_link = '<a href='.$reset_link.'>'.TEXT_LINK_LOSTPASSWORD.'</a>';
				$this->_checkExist($data['customers_email_address'], 'customers_email_address', TABLE_CUSTOMERS, "account_type = 0 and shop_id = ".$store_handler->shop_id, ERROR_EMAIL_ADDRESS_EXISTS. ' '.$reset_link);
			}
	...
	function _getParams() {
		...
		$js .= "addTab('adminHandler.php?load_section=address&pg=overview&adID='+edit_id,'".TEXT_ADDRESS." '+edit_name,'customerAddresses_'+edit_id);";
		...
		$js .= "addTab('adminHandler.php?load_section=order&pg=overview&c_oID='+edit_id,'".TEXT_ORDERS." '+edit_name,'customerOrders_'+edit_id);";
		$rowActionsFunctions['orders'] = $js;

        $rowActions[] = array('iconCls' => 'dsgvo_send_admin', 'qtipIndex' => 'qtip1', 'tooltip' => TEXT_SEND_GDPA_REPORT);
        if ($this->url_data['edit_id'])
        {
        ...
        title: '".TEXT_SEND_GDPA_REPORT."',
        msg:      '<input type=\"checkbox\" id=\"send_admin\" /> ".TEXT_DSGVO_SEND_ADMIN_ASK."',
	...
	static function pageCustomerGetSalutationTplData($cust_id)
	{
		...
		if (count($salutations_order))
		{
			...
			foreach ($salutations_order as $s)
			{
				...
				switch ($s)
				{
					...
					case 'm':
						...
						break;
					case 'c':
						$text = TEXT_COMPANY_GENDER;
						break;
                    case 'n':
                        $text = TEXT_NEUTRAL_SALUTATION ;
                        break;
                    case 'z':
                        $text = TEXT_NOT_SPECIFIED ;
                        break;
					default:
						$skip = true;
				}
			...
			$gender_data = array(array('id'=>'m', 'text'=>TEXT_MALE), array('id'=>'f', 'text'=>TEXT_FEMALE), array('id'=>'n', 'text'=>TEXT_NEUTRAL_SALUTATION),array('id'=>'z', 'text'=>TEXT_NOT_SPECIFIED));
