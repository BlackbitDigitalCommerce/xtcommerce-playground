...
$xtLink->_redirect($xtLink->_link(array('page'=>'index')));
	...
	else
    {
        ...
        if(!empty($_REQUEST['withStockOnly']) || (!empty($_SESSION['session_withStockOnly']) && array_key_exists('session_withStockOnly', $_REQUEST['session_withStockOnly'])))
        {
            $list->sql_products->setFilter('Stock');
            $_SESSION['session_withStockOnly'] = 1;
        }
        else if (!empty($_REQUEST['withStockOnly'])){
