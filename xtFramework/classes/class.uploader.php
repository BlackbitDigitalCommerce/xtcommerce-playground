...
class uploader {
	...
	function uploadFile() {
		...
		if (!is_writeable($this->upload_dir)) {
			$this->error = ERROR_DIR_NOT_WRITEABLE.' '.$this->upload_dir;
			return false;
		}
		...
		if (!$this->overwrite && is_file($this->upload_dir.$this->name)) {
		    $this->error = ERROR_FILE_EXISTS.' '.$this->upload_dir.$this->name;
		    return false;
		}
		...
		if (!$this->_isAllowedExtension()) {
		    $this->error = ERROR_FILE_NOT_ALLOWED.' '.$this->extension;
		    return false;
		}
		...
		if (move_uploaded_file($this->tmp_name, $this->upload_dir.$this->name)) {
			...
			$this->error = ERROR_MOVE_UPLOAD.' '.$this->tmp_name.' > '.$this->upload_dir.$this->name;
			return false;
    ...
    function _isWhitelisted() {
        if (count($this->extensions)>0) {
            if (in_array($this->extension,$this->extensions)) {
                ...
                $this->error = __define("ERROR_EXTENSION_NOTALLOWED");
                return false;
            }
        }
    }
