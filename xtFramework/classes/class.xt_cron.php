...
class xt_cron extends xt_backend_cls
{
    ...
    function _getParams()
    {
        ...
        if (!$this->url_data['edit_id'] && $this->url_data['new'] != true)
        {
            ...
            $rowActions[] = array('iconCls' => 'cron_start', 'qtipIndex' => 'qtip1', 'tooltip' => TEXT_CRON_START);

            $js = "
            Ext.Msg.confirm('" . TEXT_START . "','" . TEXT_START_ASK . "',function(btn){ if(btn == 'yes') runCron(record.id); })
                ...
                Ext.Ajax.request({
                    ...
                    success: function(xhr)
                    {
                        ...
                        if (typeof(json)=='object' && json.success==true)
                        {
                            ...
                            Ext.MessageBox.alert('Message', '".TEXT_CRON_STARTED."');
                        }
                        else {
                            Ext.MessageBox.alert('Message', '".TEXT_ERROR_MESSAGE."');
                        }
                    },
                    failure: function(){
                        Ext.MessageBox.alert('Message', '".TEXT_ERROR_MESSAGE."');
