...
class check_fields{
    ...
    function _checkTitle($title)
    {
        ...
        {
            $this->error = true;
            $info->_addInfo(TEXT_TITLE_MISSING);
        }
        else if (!empty($title))
        {
            ...
            {
                $this->error = true;
                $info->_addInfo(TEXT_TITLE_NOT_ALLOWED);
            }
        }
    ...
    */
        ...
        if($minAge)
        {
            try{
                ...
                if($dt_dob)
                {
                    ...
                    {
                        $this->error = true;
                        $info->_addInfo(sprintf(ERROR_MIN_AGE, $minAge));
                    }
                }
            }
            catch(Exception $e)
	...
	function _checkVatId($data,$error_message,$return_val = false) {
		...
		try {
			...
			$this->error = true;
			$info->_addInfo(ERROR_VAT_ID_SERVICE. ' ['.$e->getMessage().']');
